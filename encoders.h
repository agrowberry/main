...
volatile byte state_r;
volatile long count_l;
volatile byte state_l;
...
ISR( INT6_vect ) {
  ...
  // State:  new B   new A   old B   old A
    state_r = state_r | ( e0_B  << 3 );
    state_r = state_r | ( e0_A  << 2 );
...
ISR( PCINT0_vect ) {
    ...
    state_l = state_l | ( e1_B << 3 );
    state_l = state_l | ( e1_A << 2 );
...
void setupEncoder0() 
{
    ...
    state_r = state_r | ( e0_B << 1 );
    state_r = state_r | ( e0_A << 0 );
...
void setupEncoder1() 
{
    ...
    // Shift values into correct place in state.
    // Bits 1 and 0  are prior states.
    state_l = state_l | ( e1_B << 1 );
    state_l = state_l | ( e1_A << 0 );
